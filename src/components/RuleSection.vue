<template>
  <section class="w-full px-2.5 sm:px-9 mt-12 sm:mt-52">
    <div class="bg-[#BD5310] p-2.5 sm:p-6 rounded-xl max-w-[1098px] mx-auto relative">
      <div
        class="flex flex-col space-y-3 sm:space-y-8 px-7 sm:px-24 p-9 sm:pt-24 sm:pb-72 rounded bg-[#FBDCAB] font-noto-sans-tc text-base sm:text-[28px] relative responsive-box-shadow"
      >
        <h2
          class="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-xl sm:text-6xl font-black font-noto-sans-tc text-white grid grid-cols-[1fr_max-content_max-content_max-content_max-content_1fr] justify-center bg-gradient-to-b from-[#00C5C5] to-[#007187] border-1 sm:border-[6px] border-[#007187] rounded-xs sm:rounded-md px-3 sm:px-10 py-1 sm:py-6 gap-1 sm:gap-4"
        >
          <div></div>
          <div>活</div>
          <div>動</div>
          <div>辦</div>
          <div>法</div>
          <div></div>
        </h2>

        <p>1. 若您註冊 LTrust 帳號是以第三方登入註冊時（Google、Line或Instagram，如下圖1紅框所示），請完成下列步驟，方可登入活動網頁查看資訊：</p>

        <TextList
          :contents="[
            '請先至「登入頁面」按下「忘記密碼」；',
            '輸入您第三方登入的「 Email 帳號」，及「圖片驗證碼」後，按下「確認」按鈕；',
            '請至您的 Email 找到「會員密碼通知」信，並點選「設定新密碼」變更您的密碼；\n※若未在「收件夾」中找到信件，可前往至「垃圾信件」中查看。',
            '變更密碼完成後至本活動網頁，按下「登入」按鈕；',
            '登入您的帳號與變更後的密碼，即可查看您的活動紅利資訊。',
          ]"
        />

        <div class="flex justify-center my-4">
          <img :src="rule1Img" alt="第三方登入示意圖" class="max-w-full h-auto rounded-lg shadow-md" />
        </div>

        <br />

        <p>2. 若您是透過LTrust平台直接使用 Email 註冊帳號時（非透過第三方，如下左圖紅框所示），可直接在本活動網頁，按下「登入」按鈕，登入您的帳號與密碼，即可查看您的活動紅利資訊。</p>

        <div class="flex justify-center my-4">
          <img :src="rule2Img" alt="Email直接註冊示意圖" class="max-w-full h-auto rounded-lg shadow-md" />
        </div>

        <br />

        <p>3. 本活動以6天為一週期，每週三中午11:59:59結算該週成績，每週活動時間如下：</p>

        <TextList class="!space-y-0" />

        <p>4. 每週成績結算後，將於每週四上午10:00:00更新成績於活動頁統一公告。</p>

        <p>
          5.
          校際戰績以「學校」為計算單位，採計每週活動期間產生的「任務紅利」累積加總，為總成績計算依據。
        </p>

        <TextList
          :contents="[
            '如：A高中當週累積總任務紅利為 9,999；B高中當週累積總任務紅利為 1,111；C高中當週累積總任務紅利為 6,666；D高中當週累積總任務紅利為 8,888，則當週前3名依序為：A高中、D高中、C高中。',
            '每週前3名的學校，該校的會員每帳號依學校名次可獲得：1000點獎勵紅利、500點獎勵紅利、100點獎勵紅利。',
            '獎勵紅利統一於活動下週三晚上23:59:59前歸戶完成，可至活動頁領取。\n 如：第2週獎勵紅利，統一於2025/07/30 (三) 晚上23:59:59前歸戶。',
          ]"
        />

        <p>
          6.
          個人戰績以「個人帳號」為計算單位，採計每週活動期間產生的「任務紅利」累積達500時，將可額外獲得500「獎勵紅利」。
        </p>

        <TextList
          :contents="[
            '獎勵紅利統一於活動下週三晚上23:59:59前歸戶完成，可至活動頁領取。\n 如：第2週獎勵紅利，統一於2025/07/30 (三) 晚上23:59:59前歸戶。',
          ]"
        />

        <p>7. 相關紅利定義：</p>

        <TextList
          :contents="[
            '任務紅利：經完成LTrust平台派發任務後，所獲得的紅利。本紅利做為每週「校際戰績」與「個人戰績」成績計算。',
            '獎勵紅利：每週「校際戰績」前3名或「個人戰績」達500紅利時，所額外獲得的紅利。本紅利僅作獎勵使用，不列入「校際戰績」與「個人戰績」成績計算。',
          ]"
        />

        <p>
          8.
          當每一週開啟新的比賽時，成績皆以當週新獲得的「任務紅利」計算；過去週次比賽的「任務紅利」將不會列入當週新的比賽成績。
        </p>

        <p>9. 其他注意事項：</p>

        <TextList
          :contents="[
            '參加本活動的「校際戰績」須在賽前填寫「基本資料」中的「學校名稱」，方可參賽計算成績。\n「基本資料」中的「學校名稱」若有進行設定變更，將於7天後才能再次設定變更。\n如：王小明於2025/07/10 (四) 上午10:00:00設定變更，下次可設定變更的時間為 2025/07/17 (四) 上午10:00:00。\n活動期間內設定變更「學校名稱」者，則該週不予計算「校際戰績」，「個人戰績」仍可計算。\n如：王小明於2025/07/18 (五) 上午10:00:00 設定變更「學校名稱」，則第2週「校際戰績」不予列入該校戰績。唯「首次」設定學校名稱者不在此限。',
          ]"
        />

        <p>
          10. 活動紅利僅可兌換本平台相關服務，不得要求更換內容或轉售他人，或折換現金或其他等值商品。
        </p>

        <p>11. 本活動因故無法進行，主辦單位有權決定取消、終止、修改或暫停活動。</p>

        <p>12. 主辦單位保有任何修改本活動條款及細則之權益。</p>
      </div>
    </div>
  </section>
</template>

<script setup lang="ts">
import TextList from '@/components/ui/list/TextList.vue'
import rule1Img from '@/assets/rule1.png'
import rule2Img from '@/assets/rule2.png'

// TODO adjust header py
</script>

<style scoped>
/* 響應式 box-shadow */
.responsive-box-shadow {
  box-shadow: inset 0 0 0 6px rgba(77, 17, 17, 0.9);
  /* 小螢幕預設 8px */
}

/* 大螢幕時使用 16px */
@media (min-width: 640px) {
  .responsive-box-shadow {
    box-shadow: inset 0 0 0 16px rgba(77, 17, 17, 0.9);
  }
}
</style>
