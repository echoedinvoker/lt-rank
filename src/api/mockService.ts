...
// 靜態 mock 資料
const staticMockData: Record<string, any> = {
  '/login': {
    status: true,
    data: {
...
// 動態生成學校積分資料的邏輯
const generateSchoolBonusResponse = (data: any) => {
  ...
  // 根據學校名稱查詢
  if (schoolName) {
    const schoolData = mockSchoolData.find(s =>
      s.name.includes(schoolName) || schoolName.includes(s.name)
    )
