<template>
  <div class="flex items-center justify-around w-full gap-2 sm:gap-8 relative z-10">
  ...
  <div class="grid grid-cols-[1fr_3fr] lg:grid-cols-[max-content_1fr_max-content_1fr] gap-3
    sm:gap-4 justify-items-center items-center w-full">
      <div class="text-card">{{ `No.${index + 1}` }}</div>
      <div class="text-card">{{ record.school_name }}</div>
      <div class="bold-text px-2 leading-4.5 sm:leading-8 text-center">
        累積紅利
      </div>
